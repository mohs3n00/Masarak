  ...
  return (
    <AnimatedDiv variant="fadeUp" className="h-full">
      <Link href={`/teachers/${teacher.id}`} className="block h-full group">
        <div className={cn(
          "relative flex flex-col items-center p-4 md:p-5 rounded-2xl bg-card border border-border/40",
          "transition-all duration-300 hover:shadow-lg hover:-translate-y-1.5 hover:border-border/60 h-full",
          className
        )}>
          {/* Topographic Background Pattern */}
          <div 
            className="absolute inset-0 opacity-[0.03] dark:opacity-[0.02] pointer-events-none rounded-2xl" 
            style={{ 
          ...
          <div className="w-full aspect-square relative mb-5 overflow-hidden rounded-tl-[2.5rem] rounded-br-[2.5rem] rounded-tr-xl rounded-bl-xl shadow-sm transition-transform duration-700 group-hover:scale-[1.03] ring-1 ring-border/20">
          ...
          <div className="flex flex-col items-center text-center w-full mt-auto relative z-10">
            <h3 className="font-extrabold tracking-tight text-xl md:text-2xl text-foreground leading-tight mb-1.5 group-hover:text-primary transition-colors">
              {teacher.name}
            </h3>
            <p className="text-base md:text-[17px] font-medium text-muted-foreground/90">

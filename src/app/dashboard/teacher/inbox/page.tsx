...
import { MessageCircle, Search, Filter, Inbox, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="h-[calc(100vh-80px)] flex bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl overflow-hidden m-4 shadow-sm">
      ...
      <div className="w-1/3 max-w-[350px] min-w-[280px] flex flex-col border-l border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 shrink-0">
        ...
        <div className="p-4 border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 sticky top-0 z-10">
          <h2 className="text-xl font-bold mb-3">البريد الأكاديمي</h2>
          <div className="relative mb-3">
            <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400" size={16} />
            <input 
              type="text" 
              placeholder="البحث في الرسائل..." 
              className="w-full pl-4 pr-10 py-1.5 bg-neutral-100 dark:bg-neutral-800 rounded-full text-sm outline-none focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            <button 
              onClick={() => setActiveFilter('ALL')}
              className={`px-3 py-1 text-xs font-medium rounded-full whitespace-nowrap transition-colors ${activeFilter === 'ALL' ? 'bg-primary text-white' : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-300'}`}
            >
              الكل
            </button>
            <button 
              onClick={() => setActiveFilter('WAITING_REPLY')}
              className={`px-3 py-1 text-xs font-medium rounded-full whitespace-nowrap transition-colors ${activeFilter === 'WAITING_REPLY' ? 'bg-red-500 text-white' : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-300'}`}
            >
              بانتظار الرد
            </button>
            <button 
              onClick={() => setActiveFilter('ANSWERED')}
              className={`px-3 py-1 text-xs font-medium rounded-full whitespace-nowrap transition-colors ${activeFilter === 'ANSWERED' ? 'bg-green-500 text-white' : 'bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-300'}`}
            >
              تمت الإجابة
            </button>
          </div>
          ...
          {conversations.length === 0 ? (
            ...
            <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
              {conversations.map(conv => (
                ...
                >
                  <div className="w-10 h-10 bg-blue-100 text-blue-700 rounded-full flex items-center justify-center shrink-0 font-bold text-sm">
                    {conv.student?.name?.[0] || 'ط'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <h4 className={`text-sm truncate ${conv.unreadTeacherCount > 0 ? 'font-bold text-neutral-900 dark:text-white' : 'font-semibold text-neutral-700 dark:text-neutral-300'}`}>
                        {conv.student?.name || 'طالب مجهول'}
                      </h4>
                    ...
                    <p className={`text-xs truncate ${conv.unreadTeacherCount > 0 ? 'text-neutral-800 dark:text-neutral-200 font-medium' : 'text-neutral-500'}`}>
                      {conv.lessonSnapshot ? `بخصوص: ${conv.lessonSnapshot}` : 'استفسار عام'}
                    </p>
                  </div>
                  {conv.unreadTeacherCount > 0 && (
      ...
      <div className="flex-1 bg-white dark:bg-neutral-950 relative overflow-hidden flex flex-col">
        {selectedConversation ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-neutral-400 bg-neutral-50 dark:bg-neutral-950">
            ...
            <h3 className="text-xl font-bold text-neutral-600 dark:text-neutral-400 mb-2">البريد الأكاديمي</h3>
            <p className="text-sm">اختر محادثة من القائمة للبدء في التواصل</p>
          </div>
        )}
      </div>

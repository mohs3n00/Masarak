...
const NAV_GROUPS = [
  {
    label: 'الرئيسية',
    items: [
      { href: '/dashboard/admin', label: 'نظرة عامة', icon: LayoutDashboard, exact: true },
    ],
  },
  {
    label: 'المستخدمون',
    items: [
      { href: '/dashboard/admin/teachers', label: 'المدرسون', icon: GraduationCap },
      { href: '/dashboard/admin/students', label: 'الطلاب', icon: Users },
      { href: '/dashboard/admin/users', label: 'كل المستخدمين', icon: Shield },
    ],
  },
  {
    label: 'المحتوى',
    items: [
      ...
      { href: '/dashboard/admin/subjects', label: 'المواد', icon: Layers },
      { href: '/dashboard/admin/academic', label: 'الهيكل الأكاديمي', icon: Layers },
    ],
  },
  {
    label: 'التجارة',
    items: [
      { href: '/dashboard/admin/orders', label: 'الطلبات', icon: ShoppingBag },
      { href: '/dashboard/admin/coupons', label: 'الكوبونات', icon: Tag },
    ],
  },
  {
    label: 'الإعدادات',
...
export function AdminSidebar() {
  ...
  return (
    <aside className="fixed top-0 start-0 h-screen w-64 bg-background border-e border-border flex flex-col z-50 shadow-sm" dir="rtl">
      {/* Logo */}
      <div className="h-[68px] px-5 flex items-center shrink-0 border-b border-border">
        <Logo width={100} height={28} />
        <span className="ms-2 text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full">
          ADMIN
        </span>
      </div>
      ...
      <div className="p-3 border-t border-border shrink-0">
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-muted/50">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-text-muted">{user?.role === 'SUPER_ADMIN' ? 'مدير عام' : 'مسؤول'}</p>
          </div>
          <button

...
export default function EditCoursePage() {
  ...
  useEffect(() => {
    if (!courseId) return;
    apiClient.get(`/teacher/courses/${courseId}`)
      .then(({ data }) => {
        setForm({
          title: data.title || '',
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-text-muted">جاري تحميل بيانات الكورس...</p>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <h2 className="text-2xl font-black text-foreground mb-2">تم تعديل الكورس بنجاح!</h2>
